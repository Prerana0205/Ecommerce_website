...
function EditBanner() {
  ...
  useEffect(() => {
    const fetchBannerData = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/banners/${id}`);
        const { bannerName, status } = response.data;
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.put(`http://localhost:3001/banners/${id}`, formData, {
        headers: {
                ...
                <div className="col-lg-6 mx-auto">
                  <div className="card">
                    <div className="card-body">
                      <h5 className="card-title">Edit Banner</h5>
                      <form role="form" className="text-start">
                        <div className="input-group input-group-outline my-3">
                          <label className="form-label">Banner Name</label>
                          <input type="text" value={bannerName} onChange={(e) => setBannerName(e.target.value)} className="form-control" />
                        </div>
                        <div className="input-group input-group-outline mb-3">
                          <label className="form-label">Banner Image</label>
                          <input type="file" className="form-control" id="bannerImage" accept="image/*" onChange={handleImageChange} />
                        ...
                        <div className="text-center">
                          <button type="button" onClick={handleSubmit} className="btn bg-gradient-primary w-100 my-4 mb-2">Update</button>
                        </div>
                      </form>
                    </div>
                  </div>
